function  [ss,avr,moes]=FscatJit2_CombinedControls(identifiers, data, varargin)
...
if strcmp(barstate, 'off') && strcmp(isPaired, 'N')
        ...
        if idx == 1 
        [s1] = scatjit_CombinedControls(curDat, jitFactor, 1 ,circleSize,colors{idx});
        elseif idx ==2 
        [s1] = scatjit_CombinedControls(curDat, jitFactor, 1 ,circleSize,colors{idx});  
        else
        [s1] = scatjit_CombinedControls(curDat, jitFactor, idx-1 ,circleSize,colors{3});  
        end
            end
        else
...
%         end
        ...
        if strcmp(barstate, 'off')
            ...
            for idx = 1:length(celld_backup)
                ...
                if mod(idx,3)==0
                   location = location+1; 
                   [s2] = scatjit_CombinedControls(curDat, jitFactor, location,circleSize,colors{3});
                   location = location+1;
                else
                    [s2] = scatjit_CombinedControls(curDat, jitFactor, location,circleSize,colors{color_id});
                end
            end
